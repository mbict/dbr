...
//
// These are a set of helpers that just call LoadOne and return the value.
...
//
//    var count int64
//    err := d.Select("COUNT(*)").From("users").Where("x = ?", x).LoadOne(&count)
//
// Downsides:
...
//  - no functionality that you can't achieve calling .LoadOne directly.
